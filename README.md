...
[![Skill Version](https://img.shields.io/badge/version-1.0-blue)](./skills/ielts-writing-corrector/SKILL.md)
[![License](https://img.shields.io/badge/license-MIT-green)](./LICENSE)
[![Lines](https://img.shields.io/badge/SKILL.md-254_lines-lightgrey)](./skills/ielts-writing-corrector/SKILL.md)

---

## 📖 简介

IELTS Writing Corrector 是一个 AI Agent 雅思写作批改技能。模拟资深雅思考官的评分逻辑，对 Task 1（学术类图表 / 培训类书信）和 Task 2（议论文）作文进行全面评估，输出：
...
将本目录复制到你的 AI Agent 技能目录：

```bash
# 项目级
cp -r ielts-writing-corrector /your-project/.agents/skills/

# 用户级（全局可用）
cp -r ielts-writing-corrector ~/.agents/skills/
```

Agent 自动发现并激活。支持 ZCode、Claude Code、Cursor 等主流平台。

### 使用

向 AI Agent 提交雅思作文即可自动触发技能。支持多种输入方式：
...
└── skills/
    └── ielts-writing-corrector/
        ...
        ├── references/
        │   ├── band-descriptors.md   ← 官方 0-9 分评分细则
        │   └── common-errors.md      ← 37 个标准化错误标签
        └── tests/
            ├── test-results.md       ← 4 个测试用例完整记录
...
> 📚 完整 0-9 分细则见 [`skills/ielts-writing-corrector/references/band-descriptors.md`](skills/ielts-writing-corrector/references/band-descriptors.md)
...
> 📋 完整标签表见 [`skills/ielts-writing-corrector/references/common-errors.md` §7](skills/ielts-writing-corrector/references/common-errors.md)
...
编辑 `skills/ielts-writing-corrector/references/common-errors.md` §7 速查表，按现有格式追加一行：
...
编辑 `skills/ielts-writing-corrector/references/band-descriptors.md`，确保与 [IELTS Public Band Descriptors](https://www.ielts.org/-/media/pdfs/writing-band-descriptors-task-1.ashx) 保持一致。
...
1. 准备一篇雅思作文作为测试输入
2. 在 ZCode 中提交，触发技能
3. 对比输出与 `skills/ielts-writing-corrector/tests/test-results.md` 中记录的期望行为
4. 记录差异到 `skills/ielts-writing-corrector/tests/test-results.md` 并更新技能
